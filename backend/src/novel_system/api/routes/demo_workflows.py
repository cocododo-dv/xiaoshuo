...
def dragon_xianxia_status(
    ...
    return ok(
        DragonXianxiaDemoService(session).status(book_id=book_id, profile_id=profile_id),
...
def run_dragon_xianxia_demo(payload: dict, request: Request, session: Session = Depends(get_session)):
    ...
    result, status = execute_with_idempotency(
        ...
        action=lambda: DragonXianxiaDemoService(session).run(
            ...
            force_rerun=bool(request_payload.get("force_rerun")),
        ),
        actor_ref=actor_ref,
    )
