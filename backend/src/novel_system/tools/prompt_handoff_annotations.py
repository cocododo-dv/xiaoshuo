...
CALL_SITES: list[dict[str, Any]] = [
    ...
    _cs(f"{SVC}/scene_quality.py", 'node_id="scene_auto_rewrite"', ["scene_auto_rewrite"], "场景自动改写候选（内联提示词）", occurrence=2),
    _cs(
    ...
    _cs(f"{SVC}/literary_eval.py", "self.llm_client.generate(self._request(case))", ["literary_eval_live"], "文学评测 live 场景生成（内联提示词）"),
    # -- system_config 裸 HTTP（3 处，绕过 LLMClient 但走同一套 provider adapter）--
    _cs(f"{SVC}/system_config.py", "response = httpx.post(", [], "连通性探针：最小补全请求（_probe_completion，无业务提示词）"),
...
UNITS: list[dict[str, Any]] = [
    ...
    *[_snowflake_unit(k, l, o) for k, l, o in _SNOWFLAKE_STEPS],
    ...
    },
    {
        ...
        "call_chain": [(f"{SVC}/scene_quality.py", 'node_id="scene_auto_rewrite"', 2, "_generate_llm_candidate（occurrence 1 是离线审计行）")],
    ...
    },
    {
        ...
        "trigger": "同上。",
        "call_chain": [(f"{SVC}/style_reference/extractors/base.py", "call_llm_node(node_id", 1, "extract_node_id=本节点")],
    ...
    },
    {
        ...
        "call_chain": [(f"{SVC}/literary_eval.py", "self.llm_client.generate(self._request(case))", 1, "LiveLiteraryEvalGenerator.__call__")],
...
NEGATIVE_EVIDENCE: list[str] = [
    ...
    "`services/llm_providers/` 手写 adapter 构造裸 HTTP payload，业务出口唯一收敛在 `LLMClient.generate`。",
