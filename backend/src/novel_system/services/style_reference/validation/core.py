...
def _load_plagiarism_corpus(repo, book_id: str) -> list[str]:
    ...
    if cached is not None:
        ...
        return cached
    texts = [paragraph.text for paragraph in repo.list_paragraphs(book_id) if paragraph.text]
...
def _compute_full_verdict(
    *,
    ...
    quant_pass_rate = sum(1 for item in quant if item.passed) / len(quant) if quant else 1.0
    semantic_mean = sum(item.score for item in semantic) / len(semantic) if semantic else 6.0
    if quant_pass_rate >= 0.8 and semantic_mean >= 6.0:
        return ValidationVerdict.PARTIAL if semantic_degraded else ValidationVerdict.PASS
