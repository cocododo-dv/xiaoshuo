...
def test_reindex_rejects_stale_job_target_after_new_candidate_claim(client, session) -> None:
    promote_active_alias(client)

    review_id_old = import_style_review(
        client,
        review_id="review_style_old_reindex",
        lineage_key="STY_OLD_REINDEX",
        candidate_text="old reindex should not overwrite the new collection",
        active_on_approve=1,
    )
    approve_review(client, review_id_old, idempotency_key="approve-review-style-old-reindex")
    old_jobs = job_ids_for_review(client, review_id_old)

    review_id_new = import_style_review(
        client,
        review_id="review_style_new_reindex",
        lineage_key="STY_NEW_REINDEX",
        candidate_text="new reindex owns the current candidate collection",
        active_on_approve=1,
    )
    new_row_id = approve_review(client, review_id_new, idempotency_key="approve-review-style-new-reindex")
    ...
    with pytest.raises(DomainError) as exc_info:
        VersionManager(session)._run_reindex(old_jobs["reindex"])
