...
def _seed_project(session, project_id="proj_bp"):
    ...
    session.add(
        StoryProject(
            project_id=project_id, title="蓝图测试项目", outline_text="测试大纲"
        )
    )
    session.flush()
...
def _seed_scene(
    session, scene_id="sc01", chapter_id="ch01", project_id="proj_bp", **kwargs
):
    defaults = dict(
        scene_id=scene_id,
        chapter_id=chapter_id,
...
class TestStyleDriftDetector:
    ...
    def test_structured_profile_baseline_uses_mean_instead_of_degrading(self, session):
        from novel_system.services.style_drift_detector import detect_chapter_drift
        ...
        session.add(
            FinalScene(
                row_id="fs_structured_drift",
                scene_id="sc01",
                chapter_id="ch01",
                content="他停下。她回头。门忽然开了。" * 20,
                status="approved",
                source_bundle_id="bundle_test",
                source_bundle_hash="hash_test",
            )
        ...
        report = detect_chapter_drift(
            ...
            "ch01",
            {
                "avg_sentence_length": {"mean": 80.0, "std": 3.0},
                "question_density_per_1k": {"mean": 0.01, "std": 0.01},
            },
        )

        assert report.metrics_computed > 0
        assert report.has_drift
        assert any(item.dimension == "avg_sentence_length" for item in report.drifts)
...
class TestHookTypeClassification:
    ...
    def test_adjacent_hook_check(self):
        ...
        warnings = check_adjacent_hook_types(
            ["suspense", "suspense", "emotion", "emotion"]
        )
    ...
    def test_no_adjacent_violation(self):
        ...
        warnings = check_adjacent_hook_types(
            ["suspense", "emotion", "reversal", "info_gap"]
        )
...
class TestExpandedNarrativeEvents:
        ...
        evts = (
            session.execute(
                select(NarrativeEvent).where(
                    NarrativeEvent.event_type == "location_change"
                )
            )
            .scalars()
            .all()
    ...
    def test_foreshadow_plant_event(self, session):
        ...
        evts = (
            session.execute(
                select(NarrativeEvent).where(
                    NarrativeEvent.event_type == "foreshadow_plant"
                )
            )
            .scalars()
            .all()
...
class TestChapterTransitionBuffer:
    def test_first_scene_gets_previous_chapter_tail(self, session):
        ...
        scene2 = _seed_scene(
            session, scene_id="sc02_first", chapter_id="ch02", scene_seq=1
        )
        session.flush()
...
class TestPovVoiceColoring:
    ...
    def test_bible_fields_used(self):
        ...
        assert (
            "被遗弃" in directive.narration_guidance
            or "healer" in directive.narration_guidance.lower()
        )
...
class TestVoiceFingerprint:
    ...
    def test_format_qc_checklist(self):
        from novel_system.services.voice_fingerprint import (
            extract_fingerprint_from_bible,
            format_voice_fingerprint_for_qc,
        )

        fp = extract_fingerprint_from_bible(
            "测试", {"banned_words": ["绝对不会说的词"]}
        )
...
class TestThemeCounterpoint:
    ...
    def test_counterpoint_coverage_flags_stale(self, session):
        ...
        _seed_scene(
            session, scene_id="sc_ch01", chapter_id="ch01", pov_character_id="林远"
        )
        _seed_scene(
            session,
            scene_id="sc_ch05",
