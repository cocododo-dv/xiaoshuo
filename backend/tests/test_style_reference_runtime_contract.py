...
def test_style_drift_baseline_uses_frozen_contract_after_live_profile_changes(
    session,
) -> None:
    seeded = _seed_reference(session, seed="drift", strategy="A")
    contract = build_style_runtime_contract(
        seeded.repo,
        [seeded.binding],
        task_type="scene_generation",
    ...
    chapter = ChapterGoal(
        chapter_id="contract_chapter_drift",
        project_id=seeded.project_id,
        chapter_goal="保持风格基线稳定。",
    )
    scene = SceneCard(
        scene_id="contract_scene_drift",
        project_id=seeded.project_id,
    ...
    state = SceneRunState(
        ...
        current_bundle_id="contract_bundle_drift",
        current_bundle_hash="not-used-by-baseline-reader",
    )
    session.add_all(
        [
            project,
            ...
            state,
            SceneBundle(
                bundle_id="contract_bundle_drift",
                scene_id=scene.scene_id,
                chapter_id=chapter.chapter_id,
                execution_mode="P2",
                bundle_snapshot_hash="not-used-by-baseline-reader",
    ...
    profile.profile_json = {
        **dict(profile.profile_json or {}),
        "metrics_baseline": {"avg_sentence_length": {"mean": 999.0, "std": 0.0}},
    }
    seeded.binding.status = "disabled"
    session.flush()

    baseline = Orchestrator(session)._load_style_baseline(scene)

    assert baseline["avg_sentence_length"] == pytest.approx(18.0)
    assert baseline["short_sentence_ratio"] == pytest.approx(0.2)
