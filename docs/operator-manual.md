...
如果你第一次接触这套系统，先不要试图一次理解全部 11 个菜单。按下面这条最短闭环走就够了：

1. 先在左侧 `界面模式` 选择 `作家`。
   这是当前默认的写作者模式，会弱化内部 ID、hash、payload、JSON/YAML 等技术细节；只有排查运行链路时才需要切到 `高级`。
2. 打开 `1 配置环境（系统配置）`。
   确认 `API 地址` 指向当前后端，必要时填写 `操作员标识`。如果你要跑真实模型，再看 `模型接入` 和 `节点路由矩阵`。
3. 打开 `2 编排章节（作者工作台）`。
   选一个现有章节，或者新建章节与场景。这里的目标不是写正文，而是把“这一章/这一场要做什么”说清楚，尤其要补好章节和场景的 `戏剧卡`。
4. 在 `2 编排章节（作者工作台）` 的场景列表里点击“在场景工作台打开”。
   把某个具体场景交给运行时。
5. 在 `3 运行场景（场景工作台）` 点击“读取”，先看 `运行前检查`。
   如果预检允许运行，再启动场景运行；如果不允许，先补依赖或处理阻塞项。
6. 场景有正文结果后，在同页点击 `运行戏剧诊断`。
   作家诊断会给出分数、问题列表、证据摘录、可执行修订 brief 和候选修订；候选只是建议稿，不会自动覆盖终稿。
7. 如果系统产生了待处理候选或人工事件，转到 `4 处理审核（审核收件箱）`。
   在这里做 `批准`、必要时做 `发布`，让审核闭环真正完成。
8. 回到 `5 查看成稿（章节成稿中心）`。
   查看章节的 `实时拼接正文` 和 `最终聚合正文`；需要章级判断时，在 `成稿编辑台` 运行 `章节诊断`，再处理 `候选账本`。

第一次上手时，`6 长篇控制（长篇控制塔）`、`9 沉淀知识（知识控制台）`、`10 学习参考（参考书学习）`、`8 发布索引（索引控制台）`、`11 导入导出（互操作中心）` 都可以先只知道用途，不急着精通。
...
1. 在左侧 `界面模式` 选择 `作家`。
2. 到 `2 编排章节`，在 `章节表单` 填章节级戏剧卡，在 `场景表单` 填场景级戏剧卡。
3. 从场景列表点击 `在场景工作台打开`，到 `3 运行场景` 读取并运行单场景。
4. 场景有正文或草稿后，点击 `运行戏剧诊断`，阅读“这一场是否成立”卡片里的问题、证据和修订候选。
5. 到 `5 查看成稿`，在 `成稿编辑台` 点击 `运行章节诊断`，看整章节奏、场景缺口和候选账本。
6. 对候选修订点击 `采纳` 或 `拒绝`。采纳只表示“作者认可这个方向”，系统会把决定写入候选账本；它不会替换 `定稿`、`最终场景`、`实时拼接正文` 或 `最终聚合正文`。
7. 采纳后，用户下一步是人工合并候选句段、回 `2 编排章节` 调整戏剧卡后重跑，或在 `3 运行场景` / `5 查看成稿` 重新生成与聚合。
...
这块左侧导航栏就是整套系统的总入口。新版导航使用步骤名来降低上手成本：`作家` 模式只显示步骤名和主入口，减少第一次使用时的技术噪音；`高级` 模式会额外显示旧页面名、`view` 标识、缓存和分组信息，方便和历史文档、代码对象对照。默认第一屏仍是 `3 运行场景（场景工作台）`。

左侧还有 `界面模式` 切换：

- `作家`：默认模式，导航和页头都先回答“现在该做什么”，旧页面名、`view`、hash、payload、JSON/YAML 等内容默认收起，适合日常编排、诊断和修订判断。旧环境里存过的 `guided` 会自动迁移成 `writer`。
...
下面重点说明日常操作闭环中最常用的页面：做什么、为什么需要、什么时候进、页面区块、典型操作、上下游、最容易误解的点。`6 长篇控制` 先按总览理解即可，日常 writer 闭环不依赖它。
