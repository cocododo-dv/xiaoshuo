...
> 面向系统操作者、作者与轻度维护者
> 以当前仓库中的前端页面、FastAPI 路由和本地运行中的 UI 为准
> 本手册是当前唯一维护的操作者入口文档
>
> ⚠️ 前端主线已切换:`start-dev.cmd` 默认打开 React「潮汐工作台」(`http://127.0.0.1:5174`),
> 旧版 Vue 界面(`http://127.0.0.1:5173`)随之一并启动、仅作 legacy 备用。下方逐屏操作步骤
> 目前仍以 legacy Vue 界面为准;React 主线提供同构的「作家 / 高级」分模式与等价流程,逐屏改写待补。

## 这套系统到底在解决什么问题

这不是一个“只负责写正文”的编辑器，而是一套把小说创作拆成多个可治理环节的共享控制台。当前普通作家主线已经切到 `雪花工作台`：先把小说想法扩成十步雪花，再把场景列表和场景规划整理成章节/场景结构，之后再进入正文小修、审核、参考学习和高级运行链路。
...
1. 在仓库根目录运行 `.\start-dev.cmd`。它会自动打开 React 主线「潮汐工作台」`http://127.0.0.1:5174`(当前默认前端);旧版 Vue 界面 `http://127.0.0.1:5173` 也会一并启动,本节后续步骤以该 legacy Vue 界面为准。
...
| `outline_plan` | 结构计划 | 雪花工作台 | 它是把已确认的场景列表/场景规划整理成章节结构后的待确认计划。 |
...
- 把已确认的场景列表和场景规划整理为结构计划，并最终确认成 `ChapterGoal` 和 `SceneCard`。
...
这份手册就是当前版本的操作者入口说明，应与仓库里的前端页面、FastAPI 路由和本地运行中的 UI 保持同步。后续如果系统行为更新，应优先更新这里，而不是维护另一份并行入口文档。
