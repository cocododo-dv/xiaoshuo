...
9. 结构确认会创建 `ChapterGoal` 和 `SceneCard`。需要正文微调时进入 `小修写作`；需要参考画像时进入 `参考书学习`；出现需要人工决策的建议时进入 `待处理建议`。
10. 逐章运行、终稿批准、场景运行证据、索引发布和互操作回放属于支撑接口/高级链路，不是当前雪花工作台的直接按钮。
...
4. 如需正文层小修，进入 `小修写作`；如需抽象风格输入，进入 `参考书学习`；如需人工决策，进入 `待处理建议`。
...
- 当前雪花工作台没有直接的 `运行本章` 或 `批准终稿` 按钮；这些能力存在于项目支撑接口和高级运行链路中。
