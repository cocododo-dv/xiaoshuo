...
test("creates and edits author source-of-truth before handing a scene off to the workbench", async ({ page }) => {
  ...
  await expect(page.getByTestId("author-workspace-view")).toContainText("Author Workspace");
  ...
  await expect(page.getByTestId("notice-stack")).toContainText("Saved chapter CH300");
  ...
  await page.getByLabel("Location").last().fill("North archive");
  await page.getByTestId("author-save-scene-button").click();
  await expect(page.getByTestId("notice-stack")).toContainText("Saved scene CH300_SC01");
  ...
  await page.getByLabel("Location").last().fill("Clock bridge");
  await page.getByTestId("author-save-scene-button").click();
  await expect(page.getByTestId("notice-stack")).toContainText("Saved scene CH300_SC02");
  ...
  await expect(page.getByTestId("scene-workbench-view")).toContainText("Location: Clock bridge");
});
