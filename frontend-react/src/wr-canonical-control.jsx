...
function WrCanonicalControl({ saveStatus, canonicalStatus, disabled = false, onPromote }) {
  ...
  const promotionDisabled = disabled || busy || status === "review" || status === "unknown" || status === "current";
  return (
    <div className="wr-publish-state" aria-label="草稿与权威正文状态">
      <span role="status" aria-live="polite" aria-atomic="true" className={`wr-save ${saveStatus === "草稿已保存" ? "" : "saving"} ${saveFailed ? "is-error" : ""}`} data-testid="draft-save-status">
        <span className="wr-save-dot" />{saveStatus}
      </span>
      <span role="status" aria-live="polite" aria-atomic="true" className={`wr-canonical-state is-${status}`} data-testid="canonical-status">
