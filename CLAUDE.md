...
Full Windows CI lane: `powershell -ExecutionPolicy Bypass -File scripts/verify_windows.ps1` (pytest `-m "not chroma_integration"` + the Vue frontend's `npm test` + build). Full release lane (Windows CI → seeded Vue Playwright E2E → WSL strict Chroma): `scripts/verify_release.ps1`.
